...
As it is unrealistic that 120 million events are created within a few seconds or minutes, the tests regarding the event processing performance of the bridge application are capped at 150,000 events.
...
Instead of reading the events from an Event Store subscription, this modified bridge just generates the events on its own and sends them via the same bulk indexing call to Elasticsearch.
