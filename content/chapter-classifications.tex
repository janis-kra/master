...
\label{subsec:system:storage:req}
...
\begin{description}
\item [HTTP Interface]
As flexibility is key in a distributed \ac{CSE} system (cf. \cref{subsec:system:storage:req}) and the storage solution in general communicates via HTTP, the data aggregation service must also have a HTTP interface.
\item [Search]
...
\item [Free License]
Just as for the storage solution (cf. \cref{subsec:system:storage:req}), the aggregation service shall also be available in some form of free or even open source license.
...
When judged in isolation of the rest of the proposed stack, Solr would also be suitable solution, but it falls behind as none of the analysis applications (cf. \cref{sec:system:analysis}) have Solr support.
...
Secondly, the two preferred analysis solutions (Kibana and Grafana, cf. \cref{sec:system:analysis}), both come with dedicated support for Elasticsearch, but \emph{not} for Solr.
